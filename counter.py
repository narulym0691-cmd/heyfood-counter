#!/usr/bin/env python3
"""
헤이푸드 유부초밥 성형기 생산 카운팅 프로그램
라즈베리파이 + 카메라 기반 영상 분석으로 생산수량 자동 집계

실행: python3 counter.py
"""
...
CONFIG = {
    ...
    "server_port": 5000,        # 웹 서버 포트
    "machine_name": "유부초밥 성형기",
    # ─────────────────────────────────────────────
    # 클라우드 PUSH 설정 (2026-05-14 추가)
    # ─────────────────────────────────────────────
    "machine_id": "yooboo",     # Firebase Realtime DB 노드 이름
    "location": "2층",
    "device_type": "counter",
    "enable_cloud_push": True,  # False면 클라우드 전송 X (로컬만)
}
...
if __name__ == "__main__":
    ...
    if _CLOUD_PUSH_AVAILABLE and CONFIG.get("enable_cloud_push", True):
        ...
        # state 변경을 주기적으로 pusher.update()로 반영 (1초마다)
        def state_sync_loop():
            while True:
                try:
